...
def decode_instruction(line):
    try:
        ...
        return int(bin_32, 2).to_bytes(4, byteorder='big')
...
def process_and_save():
    ...
    try:
        for line in lines:
            byte_data = decode_instruction(line)
            if byte_data: all_bytes += byte_data
        if not all_bytes:
            messagebox.showwarning("Aviso", "No hay nada que guardar.")
            return
...
window = tk.Tk()
window.title("Decodificador MIPS")
...
tk.Label(window, text="Soporta: R-Type, I-Type, J-Type", fg="blue").pack()
